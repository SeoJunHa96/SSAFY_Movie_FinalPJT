...
|      일정       |                           내용                           |   비고         |
| :-------------: | :------------------------------------------------------: | :-------------: | 
| ~ 11월 15일(수) |             git 생성, 프로젝트 구상 및 기획              |       완료      | 
|  11월 16일(목)  |               ERD 작성, 기본적인 구조 생성               |       완료     | 
|  11월 17일(금)  | 필요 API 가져오기, 영화 정보 JSON 생성, 프론트 개발 시작 |     완료       | 
|  11월 18일(토)  |                     핵심 기능 구현                     |     진행 중 모델링 오류로 인해 리뉴얼       | 
|  11월 19일(일)  |         1차 중간 점검(디버깅 및 보안 사항 확인)          |     기한 내에 구현 가능성 있는 부분 재 기획       | 
|  11월 20일(월)  |                     인증 관련 구현                        |     일부 구현(게시글 생성 및 프로필 접근)       | 
|  11월 21일(화)  |                      미구현 부분 구현                        |      기한 내 구현 가능성 있는 부분 구현      | 
|  11월 22일(수)  |                  배포, 디자인 마무리, 디버깅                  |     배포 제외, 현재까지 구현한 기능에 대한 디버깅 완료    | 
|  11월 23일(목)  |                  최종 마무리, 발표 준비                  |      완료      | 
...
기능 구현은 움직이는 gif파일로 제작 (https://github.com/SeoJunHa96/SSAFY_Movie_FinalPJT/tree/main/img)
...
서준하:
“개발자는 실패가 일상인 직업이다.” <br>
이걸 이해하고 받아드리려고 노력했던 프로젝트였다.<br>
확실히 이론과 실전이 다르다는 것을 뼈저리게 느꼈다. <br>
부족했던 부분들을 적나라하게 확인할 수 있었고, <br>
내가 가진 취약점들을 이번기회에 보완해서 <br>
다음 프로젝트에서는 원하는 바를 구현할 수 있기를 기대한다. <br><br>
